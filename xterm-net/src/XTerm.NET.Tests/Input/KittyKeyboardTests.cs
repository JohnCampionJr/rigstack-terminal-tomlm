...
public class KittyKeyboardTests
{
    ...
    private static readonly KittyKeyboard Kitty = new();
    ...
    public void Shift_alone_on_a_character_key_stays_plain_text()
    {
        ...
        Assert.Equal("A", Kitty.Evaluate(Ev("A", shift: true), Disambiguate));
    }
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev("a", shift: shift, alt: alt, ctrl: ctrl, meta: meta), Disambiguate));
    }

    [Fact]
    public void No_modifiers_omits_the_modifier_field()
    {
        Assert.Equal("\u001b[27u", Kitty.Evaluate(Ev("Escape"), Disambiguate));
    }
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key), Disambiguate));
    }

    [Fact]
    public void Shift_Tab_is_CSI_9_2_u()
        => Assert.Equal("\u001b[9;2u", Kitty.Evaluate(Ev("Tab", shift: true), Disambiguate));

    [Fact]
    public void Ctrl_Enter_is_CSI_13_5_u()
        => Assert.Equal("\u001b[13;5u", Kitty.Evaluate(Ev("Enter", ctrl: true), Disambiguate));

    [Fact]
    public void Alt_Escape_is_CSI_27_3_u()
        => Assert.Equal("\u001b[27;3u", Kitty.Evaluate(Ev("Escape", alt: true), Disambiguate));

    [Fact]
    public void Ctrl_Backspace_is_CSI_127_5_u()
        => Assert.Equal("\u001b[127;5u", Kitty.Evaluate(Ev("Backspace", ctrl: true), Disambiguate));

    [Fact]
    public void Ctrl_Space_is_CSI_32_5_u()
        => Assert.Equal("\u001b[32;5u", Kitty.Evaluate(Ev(" ", ctrl: true), Disambiguate));

    [Fact]
    public void Alt_Space_is_CSI_32_3_u()
        => Assert.Equal("\u001b[32;3u", Kitty.Evaluate(Ev(" ", alt: true), Disambiguate));
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key, shift: shift, ctrl: ctrl), Disambiguate));
    }

    [Fact]
    public void Ctrl_Shift_ArrowRight_is_CSI_1_6_C()
        => Assert.Equal("\u001b[1;6C", Kitty.Evaluate(Ev("ArrowRight", ctrl: true, shift: true), Disambiguate));
    ...
    public void Shift_F1_moves_to_the_CSI_form()
        => Assert.Equal("\u001b[1;2P", Kitty.Evaluate(Ev("F1", shift: true), Disambiguate));

    [Fact]
    public void Ctrl_F5_is_CSI_15_5_tilde()
        => Assert.Equal("\u001b[15;5~", Kitty.Evaluate(Ev("F5", ctrl: true), Disambiguate));
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key, code), Disambiguate));
    }

    [Fact]
    public void Ctrl_Numpad5_carries_the_modifier()
        => Assert.Equal("\u001b[57404;5u", Kitty.Evaluate(Ev("5", "Numpad5", ctrl: true), Disambiguate));
        ...
        string key, string code, bool shift, bool alt, bool ctrl, bool meta, string expected)
    {
        Assert.Equal(expected,
            Kitty.Evaluate(Ev(key, code, shift: shift, alt: alt, ctrl: ctrl, meta: meta), AllKeys));
    }
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key, key), AllKeys));
    }
    ...
    {
        Assert.Equal("a", Kitty.Evaluate(Ev("a"), Disambiguate | EventTypes));
        Assert.Equal("\u001b[27u", Kitty.Evaluate(Ev("Escape"), Disambiguate | EventTypes));
        Assert.Equal("\u001b[97;5u", Kitty.Evaluate(Ev("a", ctrl: true), Disambiguate | EventTypes));
    }

    [Fact]
    public void Repeat_suffixes_colon_two()
    {
        Assert.Equal("a", Kitty.Evaluate(Ev("a"), Disambiguate | EventTypes, KittyKeyboardEventType.Repeat));
        Assert.Equal("\u001b[27;1:2u",
            Kitty.Evaluate(Ev("Escape"), Disambiguate | EventTypes, KittyKeyboardEventType.Repeat));
        Assert.Equal("\u001b[97;4:2u",
            Kitty.Evaluate(Ev("a", shift: true, alt: true), Disambiguate | EventTypes, KittyKeyboardEventType.Repeat));
    }
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key), Disambiguate | EventTypes, KittyKeyboardEventType.Repeat));
    }
    ...
    public void Release_suffixes_colon_three()
    {
        Assert.Equal("\u001b[97;1:3u",
            Kitty.Evaluate(Ev("a"), Disambiguate | EventTypes, KittyKeyboardEventType.Release));
        Assert.Equal("\u001b[27;1:3u",
            Kitty.Evaluate(Ev("Escape"), Disambiguate | EventTypes, KittyKeyboardEventType.Release));
        Assert.Equal("\u001b[97;5:3u",
            Kitty.Evaluate(Ev("a", ctrl: true), Disambiguate | EventTypes, KittyKeyboardEventType.Release));
    }
    ...
    {
        Assert.Null(Kitty.Evaluate(Ev(key), Disambiguate | EventTypes, KittyKeyboardEventType.Release));
    }

    [Fact]
    public void Functional_key_release_keeps_the_tilde_form()
        => Assert.Equal("\u001b[3;1:3~",
            Kitty.Evaluate(Ev("Delete"), Disambiguate | EventTypes, KittyKeyboardEventType.Release));
    ...
    public void Modifier_key_release_reports_its_own_bit_cleared()
    {
        ...
        Assert.Equal("\u001b[57441;1:3u",
            Kitty.Evaluate(Ev("Shift", "ShiftLeft"),
    ...
    {
        Assert.Equal("\u001b[97;5u", Kitty.Evaluate(Ev("a", ctrl: true), EventTypes));
        Assert.Equal("\u001b[97;5:2u", Kitty.Evaluate(Ev("a", ctrl: true), EventTypes, KittyKeyboardEventType.Repeat));
        Assert.Equal("\u001b[97;5:3u", Kitty.Evaluate(Ev("a", ctrl: true), EventTypes, KittyKeyboardEventType.Release));
    }
    ...
    {
        Assert.Null(Kitty.Evaluate(Ev("Shift", "ShiftLeft", shift: true), EventTypes));
        Assert.Null(Kitty.Evaluate(Ev("Shift", "ShiftLeft"), EventTypes, KittyKeyboardEventType.Release));
    }
    ...
    {
        Assert.Null(Kitty.Evaluate(Ev(key, key), Disambiguate));
        Assert.Null(Kitty.Evaluate(Ev(key, key), EventTypes));
        Assert.Null(Kitty.Evaluate(Ev(key, key), Disambiguate | EventTypes));
    }

    [Fact]
    public void Lock_key_release_sends_nothing_without_ReportAllKeys()
        => Assert.Null(Kitty.Evaluate(Ev("CapsLock", "CapsLock"), EventTypes, KittyKeyboardEventType.Release));
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key), AllKeys));
    }

    [Fact]
    public void Uppercase_uses_the_lowercase_codepoint()
        => Assert.Equal("\u001b[97;2u", Kitty.Evaluate(Ev("A", shift: true), AllKeys));
    ...
    {
        Assert.Equal(expected, Kitty.Evaluate(Ev(key), AllKeys | EventTypes, eventType));
    }
    ...
    {
        Assert.Equal("\u001b[97;;97u", Kitty.Evaluate(Ev("a"), AllKeys | Text));
        Assert.Equal("\u001b[97;2;65u", Kitty.Evaluate(Ev("A", shift: true), AllKeys | Text));
        Assert.Equal("\u001b[53;;53u", Kitty.Evaluate(Ev("5"), AllKeys | Text));
        Assert.Equal("\u001b[53;2;37u", Kitty.Evaluate(Ev("%", "Digit5", shift: true), AllKeys | Text));
    }

    [Fact]
    public void Text_is_omitted_under_Ctrl_and_for_functional_keys()
    {
        Assert.Equal("\u001b[97;5u", Kitty.Evaluate(Ev("a", ctrl: true), AllKeys | Text));
        Assert.Equal("\u001b[27u", Kitty.Evaluate(Ev("Escape"), AllKeys | Text));
    }

    [Fact]
    public void Text_is_omitted_on_release()
        => Assert.Equal("\u001b[97;1:3u",
            Kitty.Evaluate(Ev("a"), AllKeys | Text | EventTypes, KittyKeyboardEventType.Release));
    ...
    {
        Assert.Equal("\u001b[97:65;2u", Kitty.Evaluate(Ev("A", "KeyA", shift: true), AllKeys | Alternates));
        Assert.Equal("\u001b[53:37;2u", Kitty.Evaluate(Ev("%", "Digit5", shift: true), AllKeys | Alternates));
    }

    [Fact]
    public void Unshifted_and_functional_keys_have_no_alternate()
    {
        Assert.Equal("\u001b[97u", Kitty.Evaluate(Ev("a", "KeyA"), AllKeys | Alternates));
        Assert.Equal("\u001b[27;2u", Kitty.Evaluate(Ev("Escape", shift: true), AllKeys | Alternates));
    }
    ...
    public void Alternates_and_text_compose()
    {
        Assert.Equal("\u001b[97:65;2;65u",
            Kitty.Evaluate(Ev("A", "KeyA", shift: true), AllKeys | Alternates | Text));
        Assert.Equal("\u001b[97:65;2:3u",
            Kitty.Evaluate(Ev("A", "KeyA", shift: true),
    ...
    public void Release_sends_nothing_without_ReportEventTypes()
        => Assert.Null(Kitty.Evaluate(Ev("a"), Disambiguate, KittyKeyboardEventType.Release));
    ...
    {
        Assert.Null(Kitty.Evaluate(Ev(key), Disambiguate));
    }
    ...
    {
        Assert.Equal(expected,
            Kitty.Evaluate(Ev(key, code, alt: true), Disambiguate, KittyKeyboardEventType.Press, macOptionAsAlt: true));
    }
    ...
    {
        Assert.Equal("\u001b[102;4u",
            Kitty.Evaluate(Ev("Ï", "KeyF", alt: true, shift: true), Disambiguate, KittyKeyboardEventType.Press, true));
        Assert.Equal("\u001b[102;7u",
            Kitty.Evaluate(Ev("ƒ", "KeyF", alt: true, ctrl: true), Disambiguate, KittyKeyboardEventType.Press, true));
        Assert.Equal("\u001b[102;3:3u",
            Kitty.Evaluate(Ev("ƒ", "KeyF", alt: true), Disambiguate | EventTypes, KittyKeyboardEventType.Release, true));
    }
    ...
    public void No_unwinding_when_the_host_did_not_ask()
    {
        ...
        Assert.Equal("\u001b[97;3u",
            Kitty.Evaluate(Ev("a", "KeyA", alt: true), Disambiguate, KittyKeyboardEventType.Press, false));
        // And a layout where key and code disagree (AZERTY) must follow the KEY, not the code.
        Assert.Equal("\u001b[97;3u",
            Kitty.Evaluate(Ev("a", "KeyQ", alt: true), Disambiguate, KittyKeyboardEventType.Press, false));
        // A composed character without unwinding reports its own codepoint.
        Assert.Equal("\u001b[402;3u",
            Kitty.Evaluate(Ev("ƒ", "KeyF", alt: true), Disambiguate, KittyKeyboardEventType.Press, false));
    }

    [Fact]
    public void No_unwinding_when_Option_is_not_held()
        => Assert.Equal("ƒ",
            Kitty.Evaluate(Ev("ƒ", "KeyF"), Disambiguate, KittyKeyboardEventType.Press, true));

    [Fact]
    public void Unwinding_falls_through_for_non_letter_non_digit_physical_keys()
        => Assert.Equal("\u001b[8230;3u",
            Kitty.Evaluate(Ev("…", "Semicolon", alt: true), Disambiguate, KittyKeyboardEventType.Press, true));
}
