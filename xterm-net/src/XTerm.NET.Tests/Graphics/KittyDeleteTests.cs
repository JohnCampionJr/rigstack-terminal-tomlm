...
public class KittyDeleteTests
{
    ...
    private static XTerm.Buffer.BufferCell Cell(Terminal terminal, int col, int screenRow)
    ...
    private static bool HasImage(Terminal terminal, int col, int screenRow)
        => Cell(terminal, col, screenRow).Placement is not null;
    ...
    private static int TileCount(Terminal terminal)
    {
        ...
        for (int row = 0; row < terminal.Rows; row++)
        {
            ...
            if (line is null)
                continue;
            for (int col = 0; col < line.Length && col < terminal.Cols; col++)
                if (line[col].Placement is not null)
                    count++;
        }
        return count;
    }
    ...
    public void A_repeated_number_refers_to_the_newest_image()
    {
        ...
        Assert.Equal(8, placement!.Image.PixelWidth);
    }
    ...
    public void A_positional_delete_ignores_the_scrollback()
    {
        ...
        for (int i = 0; i < terminal.Buffer.YBase; i++)
        {
            ...
            if (line is not null && line[0].Placement is not null)
                survives = true;
        }
